...
namespace AgendeX.Application.Features.Auth.Commands.Login;
...
public sealed class LoginCommandHandler : IRequestHandler<LoginCommand, AuthResponseDto>
{
	...
	public async Task<AuthResponseDto> Handle(LoginCommand request, CancellationToken cancellationToken)
	{
		...
		return await CreateAndPersistAuthResponseAsync(user, cancellationToken);
	}
	...
	private async Task<AuthResponseDto> CreateAndPersistAuthResponseAsync(User user, CancellationToken cancellationToken)
	{
		...
		AgendeX.Domain.Entities.RefreshToken refreshToken = new()
		{
			Id = Guid.NewGuid(),
			UserId = user.Id,
